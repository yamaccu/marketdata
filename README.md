日経平均、金先物、ビットコインと、`tickers.csv` に登録した日本株のローソク足データ（日足と1時間足）を、1日1回、17:15に取得します。  
...
データ取得前に `data/` 配下の既存CSVをすべて削除してから、現在の固定3資産と `tickers.csv` に登録されている日本株を再取得します。監視対象から外した銘柄の古いCSVは次回実行時に削除されます。
